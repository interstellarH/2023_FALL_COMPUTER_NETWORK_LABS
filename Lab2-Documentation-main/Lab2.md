...
Receiver需要维护一个滑动窗口，并且只接受编号在窗口内的数据报文。假设当前编号不大于`y`的数据报文都已被确认，且编号为`y+1`的数据报文未被确认，那么当前滑动窗口的编号范围是`[y+1, y+window_size]`。如果Receiver收到了窗口“左侧”（编号小的一侧）一个或多个报文，则此时可以更新滑动窗口的范围。
